...
func NewHeader(image *canvas.Image) *fyne.Container {
	btn1 := widget.NewButton("Inverter", func() { usecases.InvertImage([][]int{}) })
	btn2 := widget.NewButton("Opção 2", func() {})
	btn3 := widget.NewButton("Opção 3", func() {})
	btn4 := widget.NewButton("Opção 4", func() {})

	header := container.NewHBox(btn1, btn2, btn3, btn4)
